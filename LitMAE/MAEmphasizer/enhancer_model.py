...
import numpy as np
from .Ground_Perception_model import UNet as Background_perception_model
from .Ground_Perception_model import UTNet as Foreground_perception_model
...
class enhancer(nn.Module):
    ...
    def __init__(self):
        ...
        self.ldnet = Background_perception_model()

        self.spnet = Foreground_perception_model()
    ...
    def forward(self, x):
        ...
        x_bp,out3,out_1,out_2,out_3 = self.ldnet(x)
        t1=tt.time()-t0
        ...
        # b, c, w, h = x2.size()
        x_fp,out_s,out2 = self.spnet(x)
        t4=tt.time()-t1-t2-t3-t0

        x3 = x_bp+x_fp
        t5=tt.time()-t1-t2-t3-t4-t0

        x4 = self.relu(self.e_conv4(torch.cat([x_fp,x3],1)))
        ...
        return enhance_image,r, n, x_bp, x_fp
