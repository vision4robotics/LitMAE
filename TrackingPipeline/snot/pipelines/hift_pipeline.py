...
from snot.models.hift_model import ModelBuilderHiFT
from snot.trackers.hift_tracker import HiFTTracker
from snot.utils.bbox import get_axis_aligned_bbox
from snot.utils.model_load import load_pretrain


class DNS_HiFTTracker(HiFTTracker):
    def __init__(self, model, enhancer=None):
        super(DNS_HiFTTracker, self).__init__(model)
...
class HiFTPipeline():
    def __init__(self, args, enhancer=None):
        super(HiFTPipeline, self).__init__()
        if not args.config:
            args.config = './experiments/HiFT/config.yaml'
        if not args.snapshot:
            args.snapshot = './experiments/HiFT/model.pth'

        cfg.merge_from_file(args.config)
        self.model = ModelBuilderHiFT()
        self.model = load_pretrain(self.model, args.snapshot).cuda().eval()
        self.enhancer = enhancer
        self.tracker = DNS_HiFTTracker(self.model, self.enhancer)
