...
class DNS_SiamRPNTracker(SiamRPNTracker):
    def __init__(self, model, enhancer=None):
        super(DNS_SiamRPNTracker, self).__init__(model)
...
class SiamRPNppPipeline():
    def __init__(self, args, enhancer=None):
        super(SiamRPNppPipeline, self).__init__()
        if not args.config:
            args.config = './experiments/SiamRPN_mobilev2/config.yaml'
        if not args.snapshot:
            args.snapshot = './experiments/SiamRPN_mobilev2/model.pth'
        
        cfg.merge_from_file(args.config)
        self.model = ModelBuilder()
        self.model = load_pretrain(self.model, args.snapshot).cuda().eval()
        self.enhancer = enhancer
        self.tracker = DNS_SiamRPNTracker(self.model, self.enhancer)
