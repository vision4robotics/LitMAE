...
from snot.models.lpat_model import ModelBuilderLPAT
from snot.trackers.lpat_tracker import LPATTracker
from snot.utils.bbox import get_axis_aligned_bbox
from snot.utils.model_load import load_pretrain


class DNS_LPATTracker(LPATTracker):
    def __init__(self, model, enhancer=None):
        super(DNS_LPATTracker, self).__init__(model)
...
class LPATPipeline():
    def __init__(self, args, enhancer=None):
        super(LPATPipeline, self).__init__()
        if not args.config:
            args.config = './experiments/LPAT/config.yaml'
        if not args.snapshot:
            args.snapshot = './experiments/LPAT/model.pth'

        cfg.merge_from_file(args.config)
        self.model = ModelBuilderLPAT()
        self.model = load_pretrain(self.model, args.snapshot).cuda().eval()
        self.enhancer = enhancer
        self.tracker = DNS_LPATTracker(self.model, self.enhancer)
